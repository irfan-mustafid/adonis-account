...
export const getFirstTrue = (conditions: { label: boolean; value: boolean }[]): boolean => {
...
export const terbilang = (value: number | string | undefined): string => {
  ...
  const terbilangRekursif = (num: number): string => {
    ...
    if (num < 100) {
      ...
      const sisa = num % 10
      return sisa !== 0 ? `${puluhan} puluh ${terbilangRekursif(sisa)}` : `${puluhan} puluh`
    }
    if (num < 200) {
      const sisa = num % 100
      return sisa !== 0 ? `seratus ${terbilangRekursif(sisa)}` : 'seratus'
    }
    if (num < 1000) {
      const ratusan = terbilangRekursif(Math.floor(num / 100))
      const sisa = num % 100
      return sisa !== 0 ? `${ratusan} ratus ${terbilangRekursif(sisa)}` : `${ratusan} ratus`
    }
    if (num < 2000) {
      const sisa = num % 1000
    ...
    if (num < 1000000) {
      ...
      const sisa = num % 1000
      return sisa !== 0 ? `${ribuan} ribu ${terbilangRekursif(sisa)}` : `${ribuan} ribu`
    }
    if (num < 1000000000) {
      const jutaan = terbilangRekursif(Math.floor(num / 1000000))
      const sisa = num % 1000000
      return sisa !== 0 ? `${jutaan} juta ${terbilangRekursif(sisa)}` : `${jutaan} juta`
    }
    if (num < 1000000000000) {
      const miliaran = terbilangRekursif(Math.floor(num / 1000000000))
      const sisa = num % 1000000000
      return sisa !== 0 ? `${miliaran} miliar ${terbilangRekursif(sisa)}` : `${miliaran} miliar`
    }
    if (num < 1000000000000000) {
      const triliunan = terbilangRekursif(Math.floor(num / 1000000000000))
      const sisa = num % 1000000000000
      return sisa !== 0 ? `${triliunan} triliun ${terbilangRekursif(sisa)}` : `${triliunan} triliun`
    }
    return ''
  }
 ...
 */
export const tglTerbilangLengkap = (value: string | dayjs.Dayjs | undefined): string => {
  if (value === undefined || value === null || value === '') {
    return ''
  }
